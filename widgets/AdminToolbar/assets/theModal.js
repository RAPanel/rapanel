...
(function ($, window, document, undefined) {
    ...
    if(typeof window.navigator != 'undefined' && typeof window.navigator.appVersion != 'undefined') {
        ...
        if(matches != null && matches.length > 0)
	        chromeVer = parseInt(matches[1], 10);
    }
	$.extend($.expr[':'], {
	...
	function init(els, options) {
		...
		return {
			open: function (options) {
				...
				$('.' + localOptions.overlayClass).on('scroll', function (e) {
					if (chromeVer == 35 && !$(this).data('scrolled')) {
						$('.themodal-overlay').css('overflow', 'hidden').css('overflow', 'auto');
						$(this).data('scrolled', 1);
					}
				});
